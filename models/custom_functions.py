...
class haz_VolumeRenderer(torch.autograd.Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, a_sigmas, rgbs, deltas, rays_a, T_threshold):
        total_samples, opacity, rgb, ws = \
            vren.haz_composite_train_fw(sigmas, a_sigmas, rgbs, deltas,
                                        rays_a, T_threshold)
        ctx.save_for_backward(sigmas, a_sigmas, rgbs, deltas, rays_a,
                              opacity, rgb, ws)
    ...
    @staticmethod
    @custom_bwd
    def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_drgb, dL_dws):
        sigmas, a_sigmas, rgbs, deltas, rays_a, \
        opacity, rgb, ws = ctx.saved_tensors
        dL_dsigmas, dL_drgbs = \
            vren.haz_composite_train_bw(dL_dopacity, dL_drgb.contiguous(), dL_dws,
                                        sigmas, a_sigmas, rgbs, ws, deltas,
                                        rays_a,
                                        opacity, rgb,
                                        ctx.T_threshold)
        return dL_dsigmas, None, dL_drgbs, None, None, None


class all_VolumeRenderer(torch.autograd.Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, c_sigmas, p_sigmas, c_rgbs, p_rgbs, deltas, rays_a, T_threshold):
        total_samples, f_rgb, f_opacity = vren.all_composite_train_fw(c_sigmas, p_sigmas, c_rgbs, p_rgbs, deltas,
                                                                      rays_a, T_threshold)
    ...
    @staticmethod
    @custom_bwd
    def backward(ctx, dL_dtotal_samples, dL_df_rgb, dL_df_opacity):
        c_sigmas, p_sigmas, c_rgbs, p_rgbs, \
        deltas, rays_a, \
        f_rgb, f_opacity = ctx.saved_tensors
        dL_dc_sigmas, dL_dp_sigmas, dL_dc_rgbs, dL_dp_rgbs = \
            vren.all_composite_train_bw(
