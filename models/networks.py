...
class NGP(nn.Module):
    ...
    def density(self, x, return_feat=False):
        ...
        c_h = self.c_sigma_net(h)
        p_h = self.p_sigma_net(h)
        c_sigmas = TruncExp.apply(c_h[:, 0])
        p_sigmas = TruncExp.apply(p_h[:, 0])

        if return_feat: return c_sigmas, c_h, p_sigmas, p_h
        return c_sigmas + p_sigmas
    ...
    def forward(self, x, d):
        ...
        Outputs:
            ...
            p_rgbs: (N, 3)
        """
        c_sigmas, c_h, p_sigmas, p_h = self.density(x, return_feat=True)
        d = d / torch.norm(d, dim=1, keepdim=True)
        d = self.dir_encoder((d + 1) / 2)
        c_rgbs = self.c_rgb_net(torch.cat([d, c_h], 1))
        p_rgbs = self.p_rgb_net(torch.cat([d, p_h], 1))
        return c_sigmas, c_rgbs, p_sigmas, p_rgbs
