...
std::vector<torch::Tensor> haz_all_composite_train_fw(
    ...
    return haz_all_composite_train_fw_cu(
                c_sigmas,p_sigmas, c_rgbs,p_rgbs, deltas,
                rays_a, opacity_threshold);
}


std::vector<torch::Tensor> haz_all_composite_train_bw(
    ...
    CHECK_INPUT(dL_dc_rgb)
    CHECK_INPUT(dL_dc_ws)
    CHECK_INPUT(dL_dc_opacity)
    CHECK_INPUT(c_sigmas)
    CHECK_INPUT(p_sigmas)
    CHECK_INPUT(c_rgbs)
    CHECK_INPUT(p_rgbs)
    ...
    return haz_all_composite_train_bw_cu(
                dL_df_rgb,dL_df_opacity,
                dL_dc_rgb,dL_dc_ws,dL_dc_opacity,
                c_sigmas,p_sigmas,
                c_rgbs,p_rgbs,
                deltas, rays_a,
                f_rgb,f_opacity,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){
    ...
    m.def("haz_all_composite_train_fw", &haz_all_composite_train_fw);
    m.def("haz_all_composite_train_bw", &haz_all_composite_train_bw);
