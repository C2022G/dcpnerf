...
from .custom_functions import \
    RayAABBIntersector, RayMarcher, haz_VolumeRenderer, all_VolumeRenderer
...
def render(model, rays_o, rays_d, **kwargs):
    ...
    if kwargs.get('split', "train") == "train":
        ...
        results = __render_rays_test_all(model, rays_o, rays_d, hits_t, **kwargs)
...
@torch.no_grad()
def __render_rays_test_all(model, rays_o, rays_d, hits_t, **kwargs):
...
def __render_rays_train(model, rays_o, rays_d, hits_t, **kwargs):
    ...
    (results['vr_samples'],
     results['f_rgb'], results['f_opacity']) = \
        all_VolumeRenderer.apply(c_sigmas, p_sigmas, c_rgbs.contiguous(), p_rgbs.contiguous(), results['deltas'],
                                 rays_a, kwargs.get('T_threshold', 1e-4))

    (results['vr_samples'], results['c_opacity'],
     results['c_rgb'], results['c_ws']) = \
        haz_VolumeRenderer.apply(c_sigmas, p_sigmas, c_rgbs.contiguous(),
                                 results['deltas'],
                                 rays_a, kwargs.get('T_threshold', 1e-4))
